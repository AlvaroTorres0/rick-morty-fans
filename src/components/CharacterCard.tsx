...
import { addCharacterToFavorites, removeCharacterFromFavorites } from '../supabase/supabase.actions'
...
const CharacterCard: React.FC<Props> = ({ id, name, status, species, image, origin, isFavorite, onRemove }) => {
  const { user } = useAuth()

  async function handleAddCharacterToFavorites () {
    try {
      // await addCharacterToFavorites(user.email, id)
      if (user.email) {
        await addCharacterToFavorites(user.email, id)
      }
      toast.success('Personaje agregado a favoritos')
    } catch (error) {
      toast.error('El personaje ya está en favoritos')
    }
  }

  async function handleRemoveCharacterFromFavorites () {
    try {
      // await removeCharacterFromFavorites(user.email, id)
      if (user.email) {
        await removeCharacterFromFavorites(user.email, id)
      }
      ...
      toast.error('Error al eliminar el personaje de favoritos')
    }
  }

  return (
    <div className="card min-w-[360px] max-w-[360px] lg:min-w-[600px] lg:max-w-[600px] lg:card-side bg-base-100 shadow-xl flex">
        <figure><img src={image} className='rounded-full lg:rounded-3xl' alt="Album"/></figure>
        <div className="card-body">
        <h2 className="card-title w-full font-principal-bold text-4xl">{ name }</h2>
            ...
            <div className="card-actions mt-4">
                {
                  isFavorite
                    ? (
                      <button className="btn" onClick={handleRemoveCharacterFromFavorites}>Eliminar de favoritos</button>

                      )
                    : (
                      <button className="btn" onClick={handleAddCharacterToFavorites}>Agregar a favoritos</button>
                      )
                }
            </div>
        </div>
